...
local function getClosestTarget()
	...
	for _, player in pairs(Players:GetPlayers()) do
		if player ~= LocalPlayer and not isWhitelisted(player) then
			...
			if AimbotEnabled and char and part and hum and hum.Health > 0 and (not TeamCheck or isEnemy(player)) then
		...
		if player ~= LocalPlayer then
			...
			if esp then
				if ESPEnabled and char and head and root and hum and hum.Health > 0 and (not TeamCheck or isEnemy(player)) then
					...
					local v2, _ = Camera:WorldToViewportPoint(root.Position)
